...
function animate() {
    ...
    for (const p of points) {
        if (gravity) {
            ...
            p.vx += (Math.cos(anglemouse) * gravityforce) / p.weight;
            p.vy += (Math.sin(anglemouse) * gravityforce) / p.weight;
        }
        if (relative) {
            for (const astre of points) {
                ...
                const gravityforce = (gravityStrength ** 2 * astre.weight) / sqaredist ** 2;
